...
import { editReturnAndPendingData, getDuplicateBarcodeNumbers, getPaginatedReturnData, setPageLimit, toggleConfirmModal, toggleDetailModal } from "./ReturnAndPendingReducer/ReturnAndPendingSlice";
...
const ReturnAndPendingList = () => {
  ...
  const actions = [
    ...
    {
      doit: (item) => dispatch(editReturnAndPendingData(item)),
    },
    {
  ...
  const columns = useMemo(() => COLUMNS, [returnAndPendingData]);
  const tableData = useMemo(() => returnAndPendingData, [returnAndPendingData]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      await dispatch(getDuplicateBarcodeNumbers());
      // After getPaginatedPaymentData completes successfully
    };
    fetchData();
  }, [dispatch, pageIndex, pageSize, globalFilter, selectedStartDate, selectedEndDate, filterByDuplicateBarcode, filterByUnknownBarcode]);
          ...
          <div className="flex items-center">
            <ul className="lg:flex lg:space-x-1 lg:items-center">
              <li className="flex items-center space-x-2 mb-2">
                <span
                  className={`flex items-center justify-center px-3 py-1 text-white text-xs rounded-full bg-gray-500 cursor-pointer ${filterByDuplicateBarcode === false && filterByUnknownBarcode === false ? 'ring-2 ring-gray-900' : ''}`}
                  onClick={() => {
                    setFilterByDuplicateBarcode(false);
                    setFilterByUnknownBarcode(false);
              ...
              <li className="flex items-center space-x-2 mb-2">
                <span
                  className={`flex items-center justify-center px-3 py-1 text-white text-xs rounded-full bg-blue-600 cursor-pointer ${filterByUnknownBarcode === true ? 'ring-2 ring-blue-900' : ''}`}
                  onClick={() => {
                    setFilterByUnknownBarcode(true);
                    setFilterByDuplicateBarcode(false)
                  }}
                >
                  {totalUnknownBarcodeNumbers} Unknown Barcode Number
                </span>
              </li>
              {/* <li className="flex items-center space-x-2 mb-2">
                <span className="flex items-center justify-center px-3 py-1 text-white text-xs rounded-full bg-blue-600">
                  Order Not in Booking List
                </span>
              </li> */}
            </ul>
...
export default ReturnAndPendingList;
