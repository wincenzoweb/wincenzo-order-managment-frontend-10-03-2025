...
import { editSettingData } from "../SettingReducer/settingSlice";
    ...
    return (
        <Card className="h-fit">
                ...
                <div>
                   ...
                   w-full border-b border-b-gray-500 border-opacity-10   px-4 py-2 text-sm dark:text-slate-300  last:mb-0 cursor-pointer first:rounded-t last:rounded-b flex  space-x-2 items-center
                     capitalize rtl:space-x-reverse"
                            ...
                            {/* <Menu.Item onClick={() => {
                                dispatch(deleteSettings(set._id))
            ...
            <div className="text-slate-900 dark:text-white text-sm pt-4">
                <p>Sender Mail Id :<span className="text-blue-600"> {set?.SenderMail}</span></p>
            </div>
            <div className="text-slate-900 dark:text-white text-sm pt-4">
                <p>Sender Mail Password :<span className="text-blue-600"> {set?.SenderEmailPassword}</span></p>
            </div>
            <div className="text-slate-900 dark:text-white text-sm pt-4">
                <p>Max Pending Day :<span className="text-blue-600"> {set?.MaxPendingDelay} Days</span></p>
            </div>
        </Card>
    );
};
